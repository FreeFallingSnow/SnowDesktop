...
`snowwidget.exe`。便携版在使用 Steamworks SDK 构建时还会携带用于账户所有权验证的
`SnowDesktopSteamBridge.exe`，并将 `steam_api64.dll` 放入同一运行时目录通过私有程序集加载。
MSIX 安装版不携带 Steam 桥、Steam API DLL 或桥的附带文件，打包时会递归检查这一边界。
便携版和 MSIX 不携带创意工坊管理器；只有 Steam 包包含该管理器和完整发布工具。所有载荷都会校验
