...
struct Prompt
{
    system_control::Request& request;HWND password=nullptr,ssid=nullptr,security=nullptr;bool hidden=false;
    static INT_PTR CALLBACK Procedure(HWND w,UINT m,WPARAM wp,LPARAM lp)
    {
        ...
        if(m==WM_COMMAND&&(LOWORD(wp)==IDOK||LOWORD(wp)==IDCANCEL))
        {
            if(LOWORD(wp)==IDOK)
            {
                ...
                wchar_t secret[64]{};GetWindowTextW(self->password,secret,64);self->request.password=system_control::Secret(secret);SecureZeroMemory(secret,sizeof(secret));SetWindowTextW(self->password,L"");self->request.hostConfirmed=true;
            }
            EndDialog(w,LOWORD(wp));return TRUE;
        }
        if(m==WM_CLOSE){EndDialog(w,IDCANCEL);return TRUE;}return FALSE;
    }
};
bool Confirm(HWND owner,system_control::Request& request)
{
    const bool hidden=request.name=="network.wifi.connect"&&request.arguments.contains("hidden");
    if(hidden||system_control::RequiresPasswordPrompt(request))
    {
        ...
        Prompt prompt{request,nullptr,nullptr,nullptr,hidden};return DialogBoxIndirectParamW(GetModuleHandleW(nullptr),&form.dialog,owner,Prompt::Procedure,reinterpret_cast<LPARAM>(&prompt))==IDOK;
    }
    const char* key=request.name=="network.wifi.forget"?"controlCenter.confirmForget":request.name=="system.power.sleep"?"controlCenter.confirmSleep":request.name=="system.power.restart"?"controlCenter.confirmRestart":"controlCenter.confirmShutdown";
    request.hostConfirmed=MessageBoxW(owner,_LW(key),_LW("statusBar.controlCenter"),MB_OKCANCEL|MB_ICONWARNING|MB_DEFBUTTON2)==IDOK;return request.hostConfirmed;
}
}
struct SystemPanel::Impl
{
    ...
    std::vector<RECT> cards;float scale=1;int width=0,height=0;bool showing=false,closing=false,modal=false,destroying=false;DWORD contextProcess=0;HWND contextOwner=nullptr;ULONGLONG contextStarted=0;WPARAM closeGeneration=0;
    ...
    ~Impl(){destroying=true;HideNow();tooltip.Close();if(accessibility)accessibility->DetachWindow(window);backdrop.Reset();surface.Reset();visual.Reset();target.Reset();if(window)DestroyWindow(window);}
    bool Ensure()
    {
        ...
        accessibility=std::make_unique<WidgetAccessibilityProviderHost>([this]{return Accessible();},[this](const auto&,const auto& id){if(!showing||closing||slide.IsAnimating()||!input.Focus(id))return false;SetFocus(window);Paint();if(accessibility)accessibility->RefreshEvents();return true;},[this](const auto& request){return AccessibleAction(request);});
    ...
    {
        if(!showing||closing||slide.IsAnimating()||!model)return {};
    ...
    {
        if(!showing||closing||slide.IsAnimating()||!model||request.widgetId!=L"system-panel")return false;
    ...
    void Open(Request request)
    {
        ...
        source.prompt=[this](auto& request){tooltip.Hide();modal=true;const bool result=Confirm(window,request);modal=false;return result;};
    ...
    void FinishClose(){HideNow();if(!destroying&&(pending||afterClose))PostMessageW(window,kOpenPending,++closeGeneration,0);}
    void HideNow()
    {
        ...
        model.reset();current.reset();contextProcess=0;contextOwner=nullptr;slide.ResetHidden();if(accessibility)accessibility->RefreshEvents();
    }
    void Queue(Request request)
    {
        ...
        if(showing||closing){pending=std::move(request);if(!closing)Animate(false);}else{pending.reset();++closeGeneration;Open(std::move(request));}
    }
    bool ContextMenuActive(HWND foreground) const
    {
        if(!contextProcess)return false;
        DWORD pid=0;const auto thread=GetWindowThreadProcessId(foreground,&pid);if(pid!=contextProcess)return false;
        if(GetTickCount64()-contextStarted<1500)return true;
        GUITHREADINFO info{sizeof(info)};if(GetGUIThreadInfo(thread,&info)&&(info.flags&(GUI_INMENUMODE|GUI_POPUPMENUMODE|GUI_SYSTEMMENUMODE)))return true;
        const auto style=GetWindowLongPtrW(foreground,GWL_STYLE),extended=GetWindowLongPtrW(foreground,GWL_EXSTYLE);
        return foreground!=contextOwner&&(style&WS_POPUP)&&!(style&(WS_CAPTION|WS_THICKFRAME))&&
            (extended&WS_EX_TOOLWINDOW)&&GetWindow(foreground,GW_OWNER);
    }
    void Result(ui::InputResult result,POINT screen,bool keyboard=false)
    {
        if(result.kind==ui::InputResult::Kind::None||!model||!current)return;
        if(result.id.starts_with("tray:")&&current->tray)
        {
            const auto key=result.id.substr(5);if(result.kind==ui::InputResult::Kind::Drag){POINT local=screen;ScreenToClient(window,&local);if(!model->Drop(key,{local.x/scale,local.y/scale})&&dropOutside)dropOutside(key,screen);Arrange();Paint();return;}
            auto service=current->tray;const auto source=window,bar=current->owner;
            const auto state=service->Current();for(const auto& i:state.icons)if(i.key==key){contextProcess=i.identity.process;contextOwner=reinterpret_cast<HWND>(i.identity.window);contextStarted=GetTickCount64();}
            if(keyboard)service->Activate(key,result.kind==ui::InputResult::Kind::Context?tray::Activation::ContextKeyboard:tray::Activation::Keyboard,screen,{bar,source});
            else{const bool right=result.kind==ui::InputResult::Kind::Context;service->Activate(key,right?tray::Activation::RightDown:tray::Activation::LeftDown,screen,{bar,source});service->Activate(key,right?tray::Activation::RightUp:tray::Activation::LeftUp,screen,{bar,source});}return;
        }
        auto activeModel=model;activeModel->Invoke(result.id,result.kind==ui::InputResult::Kind::Value?std::optional(result.value):std::nullopt);if(model){Arrange();Paint();}
    }
    static LRESULT CALLBACK Procedure(HWND w,UINT m,WPARAM wp,LPARAM lp)
    {
        ...
        try
        {
            if(m==kOpenPending){if(wp!=self->closeGeneration||self->showing)return 0;if(self->pending){auto next=std::move(*self->pending);self->pending.reset();self->Open(std::move(next));}else if(auto fn=std::move(self->afterClose)){self->afterClose={};fn();}return 0;}
            ...
            if(m==WM_ACTIVATE&&LOWORD(wp)==WA_INACTIVE&&self->closing&&self->current&&reinterpret_cast<HWND>(lp)!=self->current->owner)
            ...
            {if(!self->ContextMenuActive(reinterpret_cast<HWND>(lp)))self->Animate(false);}
            if(!self->model||self->closing)return DefWindowProcW(w,m,wp,lp);
            if(m==WM_THEMECHANGED||m==WM_SETTINGCHANGE){self->paintDirty=true;self->Arrange();self->Paint();return 0;}
            if(m==WM_TIMER&&wp==1)
            {
                if(self->contextProcess&&!self->modal)
                {
                    const auto foreground=GetForegroundWindow();
                    if(foreground!=w&&foreground!=self->current->owner&&!self->ContextMenuActive(foreground)){self->Animate(false);return 0;}
                }
            ...
            if(m==WM_LBUTTONDOWN||m==WM_RBUTTONDOWN){self->tooltip.Hide();const D2D1_POINT_2F p{GET_X_LPARAM(lp)/self->scale,GET_Y_LPARAM(lp)/self->scale};if(self->input.Press(self->model->View(),p,m==WM_RBUTTONDOWN)){SetCapture(w);SetFocus(w);self->Paint();}return 0;}
            if(m==WM_MOUSEMOVE)
            {
                ...
                if(!self->input.Pressed().empty()){self->Result(self->input.Move(self->model->View(),p),screen);if(self->input.Dragging()){self->dragPoint=p;self->Tip(nullptr);SetCursor(LoadCursorW(nullptr,IDC_SIZEALL));self->Paint();}return 0;}
            ...
            if(m==WM_LBUTTONUP||m==WM_RBUTTONUP){const auto result=self->input.Release(self->model->View(),{GET_X_LPARAM(lp)/self->scale,GET_Y_LPARAM(lp)/self->scale},m==WM_RBUTTONUP);ReleaseCapture();POINT screen{GET_X_LPARAM(lp),GET_Y_LPARAM(lp)};ClientToScreen(w,&screen);self->Result(result,screen);self->Paint();return 0;}
            if(m==WM_LBUTTONDBLCLK){const auto* node=self->model->View().Hit({GET_X_LPARAM(lp)/self->scale,GET_Y_LPARAM(lp)/self->scale});if(node&&node->id.starts_with("tray:")&&self->current->tray){POINT screen{GET_X_LPARAM(lp),GET_Y_LPARAM(lp)};ClientToScreen(w,&screen);self->current->tray->Activate(node->id.substr(5),tray::Activation::DoubleClick,screen,{self->current->owner,w});}return 0;}
            if(m==WM_CAPTURECHANGED||m==WM_CANCELMODE){self->input.Cancel();return 0;}
            if(m==WM_MOUSEWHEEL){POINT point{GET_X_LPARAM(lp),GET_Y_LPARAM(lp)};ScreenToClient(w,&point);const auto* n=self->model->View().Hit({point.x/self->scale,point.y/self->scale});if(n&&n->role==ui::Role::Slider)self->model->Invoke(n->id,std::clamp(n->value+GET_WHEEL_DELTA_WPARAM(wp)/WHEEL_DELTA*.02f,0.f,1.f));else self->model->Scroll(-GET_WHEEL_DELTA_WPARAM(wp)/WHEEL_DELTA*42.f);self->Arrange();self->Paint();return 0;}
            ...
            if(m==WM_KEYDOWN)
            {
                ...
                self->Result(result,p,true);self->Paint();if(self->accessibility)self->accessibility->RefreshEvents();return 0;
            }
        }
        catch(...){WriteDiagnosticLogEntry(L"Native system panel failed; dismissing its surface");self->pending.reset();self->afterClose={};++self->closeGeneration;self->HideNow();}
...
void SystemPanel::CloseThen(std::function<void()> next){impl_->pending.reset();impl_->afterClose=std::move(next);if(impl_->showing)impl_->Animate(false);else if(auto fn=std::move(impl_->afterClose)){impl_->afterClose={};fn();}}
bool SystemPanel::IsOpen()const{return impl_->showing;}
