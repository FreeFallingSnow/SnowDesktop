...
{
enum class Placement : int { Hidden, Submenu, Root };
struct Selection
{
    std::string provider;
...
inline void Normalize(Preferences& value)
{
    ...
    for (auto item : value.selections)
    {
        ...
        if (item.provider.empty() || item.provider.size() > 2048 || item.command.size() > 2048 ||
            ...
            item.provider.find('\0') != std::string::npos || item.command.find('\0') != std::string::npos) continue;
        auto found = std::find_if(kept.begin(), kept.end(), [&](const auto& old) {
            return old.provider == item.provider && old.command == item.command;
        });
        if (found == kept.end()) kept.push_back(std::move(item));
...
{
    Preferences result;
    if (!json || !json->IsObject()) return result;
    if (const auto* enabled = json->Find("enabled"); enabled && enabled->IsBoolean()) result.enabled = enabled->boolean;
    if (const auto* rows = json->Find("selections"); rows && rows->IsArray())
        for (const auto& row : rows->array)
        {
            const auto* provider = row.Find("provider"); const auto* command = row.Find("command");
            const auto* place = row.Find("placement"); const auto* label = row.Find("label");
            if (!provider || !provider->IsString() || !command || !command->IsString() ||
                !place || !place->IsNumber() || place->number < 0 || place->number > 2 || place->number != static_cast<int>(place->number)) continue;
            result.selections.push_back({provider->string, command->string,
                label && label->IsString() ? label->string : "", static_cast<Placement>(static_cast<int>(place->number))});
        }
    Normalize(result);
    return result;
}
