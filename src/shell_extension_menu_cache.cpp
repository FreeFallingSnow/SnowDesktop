...
std::uint64_t Hash(std::span<const std::byte> bytes)
{
    ...
    for (auto byte : bytes) { value ^= std::to_integer<unsigned char>(byte); value *= 1099511628211ull; }
    return value;
}
...
std::optional<settings_ipc::Bytes> Read(const std::filesystem::path &path, std::uint64_t limit)
{
    ...
    if (!file.read(reinterpret_cast<char *>(bytes.data()), static_cast<std::streamsize>(size))) return {};
    return bytes;
}
bool Write(const std::filesystem::path &path, const settings_ipc::Bytes &bytes)
{
    ...
    auto temporary = path;
    temporary += L"." + std::to_wstring(GetCurrentProcessId()) + L"." + std::to_wstring(GetCurrentThreadId()) + L".tmp";
    {
        std::ofstream file(temporary, std::ios::binary | std::ios::trunc);
        if (!file.write(reinterpret_cast<const char *>(bytes.data()), static_cast<std::streamsize>(bytes.size())))
        { file.close(); std::filesystem::remove(temporary, error); return false; }
        file.close();
        if (!file) { std::filesystem::remove(temporary, error); return false; }
    }
...
bool Sanitize(std::vector<Entry> &entries, size_t &count, unsigned depth = 0)
{
    ...
    {
        if (++count > 2048 || entry.label.size() > 2048 || entry.provider.size() > 8192 || entry.key.size() > 8192 ||
            entry.width < 0 || entry.height < 0 || entry.width > 128 || entry.height > 128 ||
            entry.pixels.size() != static_cast<size_t>(entry.width * entry.height * 4)) return false;
        entry.token = 0;
        if (!Sanitize(entry.children, count, depth + 1)) return false;
    }
    return true;
}
...
struct PendingQueries
{
    ...
    UINT_PTR timer = 0;
    bool ticking = false;
    ~PendingQueries() { if (timer) KillTimer(nullptr, timer); }
    static PendingQueries &Current() { thread_local PendingQueries state; return state; }
    static void CALLBACK Tick(HWND, UINT, UINT_PTR, DWORD)
    {
        auto &state = Current();
        if (state.ticking) return; // Invoke acknowledgement may pump this STA.
        state.ticking = true;
        for (size_t i = 0; i < state.jobs.size();)
        {
            try
            {
                ...
                {
                    if (job.cache) job.cache->Store(job.ticket, *reply);
                    const auto token = ResolveCommand(*reply, job.reference);
                    if (token) job.session->Invoke(token, job.position);
                }
            }
            catch (...) { /* A failed query/invocation cannot escape a timer callback. */ }
            state.jobs.erase(state.jobs.begin() + i);
        }
        state.ticking = false;
        if (state.jobs.empty() && state.timer) { KillTimer(nullptr, state.timer); state.timer = 0; }
    }
    bool Add(Job job)
    {
        if (jobs.size() >= 8) return false;
        if (!timer) timer = SetTimer(nullptr, 0, 30, Tick);
...
MenuSnapshotCache::Ticket MenuSnapshotCache::Capture(const Request &source) const
{
    try
    {
        ...
        if (sample && (!request.catalogueOnly || request.context == Context::Automatic)) return {};
        if (request.paths.size() > 256) return {};
        if (!sample) request.catalogueOnly = false; // Shared exact-object key.
        ...
        for (auto &path : request.paths)
        {
            ...
            if (!file.is_absolute() || PathIsNetworkPathW(path.c_str()) ||
                GetDriveTypeW(file.root_path().c_str()) == DRIVE_REMOTE) return {};
        ...
        ticket.identity = settings_ipc::Pack(request, stamps, FileStamp(executable), GetUserDefaultUILanguage());
        ticket.epoch = Epoch();
        ticket.slot = sample ? static_cast<unsigned>(request.context) :
                              4 + static_cast<unsigned>(Hash(ticket.identity) % (Slots - 4));
        if (ticket.slot >= Slots) return {};
        return ticket;
    }
    catch (...) { return {}; }
}
std::optional<Reply> MenuSnapshotCache::Find(const Ticket &ticket, std::uint64_t now)
{
    try
    {
        if (!ticket || ticket.epoch != Epoch()) return {};
            ...
            row->second.epoch != ticket.epoch)
        {
            ...
            if (schema != Schema || identity != ticket.identity || epoch != ticket.epoch || !reply.ok ||
                !Sanitize(reply.entries, count)) return {};
            row = rows_.insert_or_assign(ticket.slot,
                MemoryRow{std::move(identity), std::move(epoch), written, stamp, std::move(reply)}).first;
        }
        if (now < row->second.written || now - row->second.written >= LifetimeMs) return {};
...
bool MenuSnapshotCache::Store(const Ticket &ticket, const Reply &reply, std::uint64_t now)
{
    try
    {
        if (!ticket || !reply.ok || ticket.epoch != Epoch() || Capture(ticket.request).identity != ticket.identity) return false;
        auto snapshot = reply;
        snapshot.error.clear();
        size_t count = 0;
        if (!Sanitize(snapshot.entries, count)) return false;
        const auto bytes = settings_ipc::Pack(DiskRow{Schema, ticket.identity, ticket.epoch, now, snapshot});
        if (bytes.size() > MaximumSnapshotBytes) return false;
        const auto path = directory_ / (std::to_wstring(ticket.slot) + L".bin");
        if (!Write(path, bytes)) return false;
        rows_.insert_or_assign(ticket.slot, MemoryRow{ticket.identity, ticket.epoch, now, FileStamp(path), std::move(snapshot)});
        return true;
    }
    catch (...) { return false; }
}
...
{
    if (!reply.ok || reference.empty() || reference.size() > 9) return 0;
    ...
    for (const auto &part : reference)
    {
        ...
        for (const auto &entry : *entries)
            ...
            {
                if (found) return 0; // Ambiguity must never select by position.
                found = &entry;
            }
...
{
    if (!session || reference.empty()) return false;
