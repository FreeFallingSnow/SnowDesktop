...
std::vector<std::filesystem::path> LocalCovers(std::uint32_t appId, bool portrait, std::stop_token stop)
{
    std::vector<std::filesystem::path> result;
    const auto steam = SteamDirectory();
...
struct LargeIconAssets::Impl
{
    ...
    {
        if (!r.importPath.empty()) return "import-" + Hash(r.importPath.wstring() + std::to_wstring(Now()) + std::to_wstring(r.generation)) + ".png";
    ...
    std::shared_ptr<LargeIconAsset> Load(const LargeIconAssetRequest& r, std::stop_token stop, std::string& error)
    {
        ...
        {
            auto result = Decode(r.importPath, 2048, output, reference, "local");
        ...
        if (r.appId)
        {
            for (const auto& local : LocalCovers(r.appId, r.portrait, stop))
            ...
            if (!r.localOnly && !stop.stop_requested() && (r.refresh || Now() >= retryAt))
            {
                ...
                for (const auto& language : {r.language, std::string("english")})
                {
                    ...
                    if (!url.empty())
                    {
                        ...
                        if (status == 200)
                        {
                            const auto temporary = directory / Wide(reference + ".download");
                            { std::ofstream file(temporary, std::ios::binary | std::ios::trunc); file.write(reinterpret_cast<const char*>(bytes.data()), bytes.size()); }
            ...
            catch (...) { error = "largeIcon.unavailable"; }
            {
                ...
                if (requestStop.stop_requested())
                {
                    ...
                    std::erase_if(work.listeners, [&](const auto& listener) {
                        const auto current = currentRequests.find(listener.itemKey);
                ...
                if (asset)
                {
                    auto bytes = [&] { std::uint64_t size = 0; for (const auto& [_, c] : cache) size += std::uint64_t(c.asset->width) * c.asset->height * 4; return size; };
                    ...
                    if (bytes() + incoming <= maxMemory) cache[key] = {asset, ++clock};
...
LargeIconAssets::LargeIconAssets(std::filesystem::path directory, std::function<void()> ready) : impl_(std::make_unique<Impl>())
{
...
void LargeIconAssets::Request(LargeIconAssetRequest request)
{
    bool notify = false;
    {
        ...
        impl_->currentRequests[request.itemKey] = request.generation;
        ...
        for (auto& [_, work] : impl_->pending)
            if (std::none_of(work.listeners.begin(), work.listeners.end(), [&](const auto& listener) {
                const auto current = impl_->currentRequests.find(listener.itemKey);
