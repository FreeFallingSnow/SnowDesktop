...
- 46 号调整列表 ItemContainer／Content 进入动画，使用行文字真实像素检查拒绝空白／半透明初帧；弱引用检查按显式设备列表身份分别覆盖音频和 Wi-Fi。Lua 预览恢复旧 onAC=false 并追加 charging=false，原生充电夹具单独覆盖，不改变原有 Lua 预览字段。主题用例只比较持久化的模式、自定义标志和颜色，不要求未存储的内部预设标记相同。
- 46 号 `scripts/build.bat` 退出 0，Release 宿主生成，无编译／链接警告（`46-batch-build.log`）。控制面板浅色中文 96 DPI／深色英文 144 DPI 各 9 状态导出成功，文字像素和两类列表释放检查通过（`46-render-controls.log`、`controls-render-46/`）；目检音频及 Wi-Fi 名称完整可见。定向和完整测试尚未运行，设备操作与桌面交互仍待实机。
- 46 号最终检查，代码提交 `f3b5f744`：定向 **8/8 通过**，60.53 秒（`46-batch-tests.log`，JUnit `test-run-8416cdf739ac4e16b27f5cefcc448a34.xml`）；`scripts/test.bat full` **120/120 通过**，157.78 秒（`46-full-tests.log`，JUnit `test-run-2a0aff34f2a8414282271e6552a1b7a4.xml`）。两次退出 0、无编译／链接警告。全量前后输入绑定 `46-before-full-validation-inputs.json`／`46-after-full-validation-inputs.json`。
- 显式 `scripts/test.bat name "^tray_live_integration$"` **1/1 通过且未跳过**，0.53 秒、退出 0、无编译／链接警告（`46-tray-live.log`，JUnit `test-run-fefce17e1e2f4ae68b767c22e303dc29.xml`）。使用真实生产 Service／Explorer Hook 和自有隐藏进程图标，验证初始重报、动态像素、隐藏状态、精确矩形、version 4 原始右键及 context 回调、旧版回调、删除和重连无重复。没有操作第三方菜单或桌面宿主。临时图标与连接已清理；Explorer 为回调安全固定加载的临时 Hook 文件保留至其退出。
- 本轮栏体、日历、托盘和资源离线证据分别为 `bar-render-45/`、`calendar-render-44/`、`tray-render-44/`、`resource-render-44/`；46 号仅调整控制页列表／原生充电夹具及 Lua 兼容测试，相关绘制输入未变，因此沿用这些图片，不写成重新导出。46 号全量另外执行生产预览回归。合并栏中央空区只是 Dock 预留，未以其声称真实 Dock 已视觉验收。
- 全量前后 1389 个受控输入哈希一致；最终宿主 SHA256 `cf4c4f333c6349c12e9239fa2fb588039ef60d5c223c38bf5cf5213feaf0bb1c`，运行时 Hook `8c41131bb2b349e9a820d00a21314b338f877d21587793e9b6eca2c772c158a8`。本轮未启动桌面宿主；卡顿根因、WPS 越界、微信实际菜单、真实拖拽、同侧合并、多屏全屏及动画继续待实机。400 MiB 不单独判为泄漏，列表自引用清理也不冒充旧卡顿修复。
...
| 充电图标内部填满绿色 | 44～46 号已接入官方 Filled 几何，栏体与控制中心浅深色实际像素已检查；真实电池待验收 | 顶栏和控制中心，浅深色可见；保留充电闪电识别 |
| 托盘应用右键菜单打开时展开面板保持 | 已按目标进程保留面板；关闭／全屏仍清理，第三方原菜单待实机 | 第三方菜单显示／关闭、面板外点击、焦点和全屏清理 |
| 托盘外露和排序改为直接拖拽；移除打开系统托盘按钮和标题，紧凑布局 | 已接入栏体与面板拖拽，共享持久化事务与取消清理；紧凑面板离线渲染及排序规则通过，真实拖拽待实机 | 固定／取消固定及排序拖拽，不经整理页；窄屏、DPI、空列表与受限降级 |
| 控制中心箭头跟随蓝色背景，使用 >／< 折线符号 | 44～46 号共用选中背景，浅深色实际控件检查通过；点击待实机 | 选中按钮与详情入口背景统一，返回和前进符号、浅深色与点击区域 |
| GPU 选择列表重复同名设备去重 | 展示层身份／有效别名去重回归与负向对照通过；保留有效同型号多卡，用户设备待验收 | 剔除没有独立有效数据的重复展示项；保留真正可用的同型号设备并区分身份 |
| 控制中心各子页先对齐 Windows 原生布局，声音页保留音量调节 | 音频／Wi-Fi 改为选择列表，保留音量静音；46 号实际浅深色首帧、列表释放与全量通过，设备操作待实机 | 统一头部／开关、可选择设备和网络列表、底部设置入口；音量／静音，空状态和长列表 |
| 日历左侧显示设置的第二历法；日期选中描边不应断续 | 已读取既有历法注释，选中描边 2 DIP；浅深色离线图片与全量通过，真实配置待实机 | 复用日历设置、日期切换；浅深色和混合 DPI 的连续描边 |
| 日历左侧显示选中日期而非始终今天；今天按钮移到月历顶栏 | 已随选择更新左侧，今天入口移至顶部；离线选择与渲染通过，真实点击待实机 | 星期／日／年月／第二历法同步选择；今天入口回到当月并选中当天，午夜更新不覆盖用户选择 |
...
| 状态栏弹窗接入边缘滑出动画 | 已使用 UiAnimationScheduler 与既有动画开关／速度，联动内容和玻璃位移；实际桌面动画待验收 | 复用现有动画设施；顶栏向下、底栏向上，关闭收回；数据刷新与子页切换不重复播放，遵循系统动画设置，快速切换和销毁安全 |
...
| 状态栏主题缺少全局预设，Dock 的“跟随组件”应改为“跟随全局主题” | 已补全预设、保留旧模式映射并更新文案；主题保存与解析回归通过，设置视觉待实机 | 跟随全局、完整全局预设、自定义；保留旧设置映射，不将旧模式静默解释成不同外观 |
| Wi-Fi 不需要已保存网络区 | 已移除界面区块，保留后端；46 号实际控件预览通过 | 展开只展示当前网卡的可用网络；管理入口交给系统设置，后端功能不因界面简化丢失 |
| 面板滚动条应贴近外缘 | 滚动视口扩至边缘，间距移入内容；实际控件离线检查通过，桌面长列表待验收 | 内部内容保留间距、滚动区域延伸至外缘；浅深色、DPI、圆角和长列表 |
| 可用 Wi-Fi 重复显示 HIT-WLAN，空名称只显示信号百分比 | 按当前网卡内 SSID／认证／加密身份去重，丢弃空名称未连接广播并保留隐藏网络连接入口；规则回归通过，实际扫描待验收 | 当前连接优先、信号取最佳，连接目标保持一致 |
| 状态栏系统菜单与快捷搜索两个开关无法关闭 | IPC 原遗漏字段已补齐；同一失败输入及 settings_controller 往返回归通过，实际设置窗口／重启待验收 | 设置控件→模型→规范化→持久化→生产栏体；关闭、重新打开设置和重启保持 |
| CPU／内存／GPU／网速信息默认关闭，不自动开启 | 缺省 false 与首次启用回归通过；保留已有用户设置 | 首次启用状态栏不自动开启四项，离线预览样本不得修改用户设置 |
| 状态栏常规右键菜单只保留“状态栏设置”和“任务管理器” | 已删去其他重复入口，编译与全量通过；菜单实机待验收 | 删除关闭、位置和主题等重复入口；左侧系统快捷菜单独立保留 |
| 左侧系统菜单电源操作必须二次确认 | 睡眠／重启／关机加入宿主确认，默认取消；后端确认边界回归通过，未实际执行关机等操作 | 睡眠／重启／关机确认后才执行，取消不发出操作，保持唯一确认入口 |
| 微信托盘右键无法唤起菜单 | 参考固定 YASB 补齐原始 down/up 与 context 回调；真实 Hook 自有图标协议检查通过，微信原菜单仍待实机 | 微信原场景，栏上与折叠菜单分别验证；不以消息已发送当作菜单成功显示 |
| Dock 靠边且与状态栏同侧时合并一条栏，Dock 居中、其他控件两侧 | 自动合并已接入，无开关；共用 AppBar、独立 Dock 原生内容与栏体主题，离线侧区／避让通过；真实 Dock、多屏及全屏待实机 | 共用背景和占位、时钟避让、DPI／窄屏、多屏、全屏与拖拽；不同侧或离边自动分离 |
| 隐藏与控制中心重复的系统托盘音量入口；三个状态图标提示不同，音量按档位显示、支持滚轮 | 已过滤三个系统控制图标 GUID，分别提供提示及音量档位，滚轮限定音量区域；离线／规则通过，真实悬停与音量待实机 | 保留同组唯一控制中心入口；分别命中网络／音量／电量提示，静音与音量档位，滚轮只在音量区域生效 |
