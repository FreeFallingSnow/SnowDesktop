...
| 点击状态栏自身空白也关闭菜单／弹窗 | `ca15c27b` 已编译：明确关闭动作，取消等待中的切换；鼠标焦点框已进一步调整，实机反馈待回收 | 空白命中发送关闭动作，不需要先点桌面或其他应用；兼容菜单嵌套循环 |
...
- 当前完整测试 117/120 通过，三项失败仍开放：组件作者 feature 清单、手动诊断筛选断言、窗口层级夹具。修正后重新完成有效完整测试；全部编译／链接警告必须处理。
