...
| 频繁刷新、提示闪烁 | 栏体提示切换／滚轮累积回归通过；托盘实际控件已验证未变化图像、提示及布局不被替换，栏体帧差仍待接入 | 离线帧差、交互状态测试；悬停实测 |
...
| 信息点击无需面板 → 最新改为简洁曲线＋指标面板 | 已接入四类独立信息面板与共享采样历史，首轮标准构建通过；离线卡片尺寸检查失败，正在定位 | 不进入控制中心；无数据明确不可用；GPU 可选择适配器 |
...
| 顶栏不用 WinUI，顶栏和面板均支持离线渲染 | 顶栏已原生；日历、控制中心和托盘已复用实际控件离线导出，信息面板／栏体仍待接入 | 栏体不初始化 XAML；面板共享宿主与主题，生产绘制路径可离线验收 |
| 弹窗四角，尤其底部圆角 | 共用主题边框及窗口区域已接入；日历／控制中心／托盘浅深色 PNG 四角已检查，玻璃与桌面仍待验证 | 透明角像素／主题与裁剪离线测试 |
...
| 用 Markdown 跟踪全部提示；原生离线渲染与其余可自动化验收 | 持续在本文件跟踪；日历／控制中心／托盘生产渲染已接入，信息面板和栏体保持开放 | 同一生产绘制路径；不能用另画的示意图冒充 |
