...
| 亮度实际成功却提示失败 | 已接入有界延迟回读、按控制和设备过滤过期结果；定向测试通过，待本轮构建及实机 | 延迟状态、快速滑动、失败后成功、设备失效 |
| 控制子页需合理分组，切换面板大小立即更新 | 已接入切页同步测量定位；布局及原生化仍待处理 | 设备列表／操作／滑条分区；切页同步测量和定位，不等待采样 |
| 顶栏边框只要面向桌面的一条边 | `43b94d67` 已编译，待离线及实机验证 | 顶部下边／底部上边；左右和屏幕外沿无边框 |
| Ctrl＋点击控制中心入口打开系统控制中心 | `43b94d67` 已编译，待交互验证 | Win+A；普通点击仍打开自有面板 |
| 左侧“更多”按钮不好看 | 已换为固定官方 Fluent `apps_16_regular` 四格图标，待本轮构建 | 保持系统快捷菜单语义与相邻搜索图标尺寸一致 |
| 点击状态栏自身空白也关闭菜单／弹窗 | 已接入明确关闭动作，并取消等待中的菜单→弹窗切换；待本轮构建及实机 | 空白命中发送关闭动作，不需要先点桌面或其他应用；兼容菜单嵌套循环 |
