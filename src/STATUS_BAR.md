...
- 当前栏体原生绘制，状态栏弹窗仍为 WinUI。已向用户说明现状并确认本次是否将所有状态栏弹窗迁移到原生；等待期间继续排查不依赖渲染框架的托盘协议、采集与菜单定位。
...
| 音乐控件作为控制中心下方的独立卡片 | 47 号已接入，共用弹窗宿主，待编译与渲染 | 独立主题圆角和玻璃、间隔透明、无媒体不占位、媒体切换、空间受限与窗口动画 |
