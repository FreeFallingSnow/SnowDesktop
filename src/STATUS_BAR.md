...
- 用户已明确：**除了设置，其他界面不要使用 WinUI 3**。已核对运行时宿主入口：设置宿主之外的独立 WinUI 宿主为状态栏弹窗及其离线预览；本轮迁移控制中心、托盘、日历和资源面板到共享原生自绘与输入设施，离线预览也必须使用同一生产绘制代码。新增 `native_ui_scene` 与 `system_panel_model` 正在实施，尚未接线／编译，不是完成状态。
...
- 最新主题要求（已澄清）：主题与材质页只保留状态栏设置链接，具体主题和场景覆盖放到状态栏设置页，与任务栏一致。**不增加右键主题选择或主题入口**，右键仍只有状态栏设置、任务管理器。设置、IPC 和每屏运行时接线已实施，待集中编译及实机验证。
- 最新媒体要求：不设媒体详情菜单；底部独立卡片压缩为单行窄条，依次是封面、标题和上一首／播放暂停／下一首。自绘布局已调整，待编译和离线验收。
- 最新提示要求：音量滚轮后当前提示应原位同步数值；状态栏和新面板已接自绘 Tooltip 和共享文字测量，待集中编译及实际交互验证。
- 复用审查：Lua 引擎已有控件节点、稳定身份输入与 UIA 基础；新原生草稿不能长期维护第二套同类规则。先收敛共享接口，再一次性迁移面板；设置、协议、设备服务与绘制分离，详见当前实施计划。
- 本条是用户实测失败记录；尚无新构建，不重复引用 51 号全量为上述问题通过。400 MB／鼠标卡顿根因仍未确认，51 号冷初始化修正后的实际点击闪退也尚无明确复验结论。
