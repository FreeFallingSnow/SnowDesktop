...
### 原生迁移集中候选 54（自动验收通过，实机待验）

- 状态栏四类弹窗和离线预览改为原生 Direct2D／DirectComposition，设置窗口保留 WinUI。输入、UIA、滑条几何、提示文字测量复用已有基础；关闭后打开的请求按代次取消，动画期间不执行错位输入。
...
- 53 号补齐捕获事件描述，第二历法允许换行，星期使用应用语言，扩大日程按钮并衔接分体按钮背景；收窄面板头文件依赖。`scripts/build.bat` 退出 0，无编译／链接警告，生成 Release 宿主（`53-native-build.log`）。离线复验与完整回归尚待运行，真实桌面与设备验收仍待用户执行。
- 53 号自动检查：浅色中文 96 DPI／深色英文 144 DPI 共 86 张生产图片及内置输入检查通过（`53-render.log`）；完整回归 **119/120 通过**，`widget_author_preview_cli` 的两项日历紧凑布局断言失败，90.06 秒、退出 1，无编译／链接警告（`53-full-tests.log`，JUnit `9a489af26fad4a74b0d67faea9297e8a`）。不能作为完整通过候选。第二次看图发现半透明关闭按钮内侧拼接被重复填充；独立代码审查另发现旧确认回调、设备恢复、媒体取消及无消费者后回读释放的生命周期缺口，下一候选集中处理。
- 54 号集中收尾：确认期间关闭显式失效旧模型、取消确认并阻止旧提交，模态循环退出后才开替代面板；菜单保留绑定实际 owner／线程或本次附近弹窗，无法证明的工具窗有限期。图形恢复释放并重绑栏体、面板和提示设备引用，保留 AppBar。媒体 WinRT 异步读取共享五秒预算并响应停止；设备采样／执行回读／释放按来源互斥，ready 控制任务优先于过期采样，最后消费者退出后回收。同步 COM／驱动调用仍不能强制抢占。
- 半透明分体按钮改为裁剪扩展形状的一次填充；追加生产像素断言与旧确认关闭回归。旧 CLI 尺寸断言改为保留独立画布、内容尺寸和圆角要求，仅允许生产描边每边一个物理像素；媒体样本按用户要求验证为总览别名。新增服务竞态使用条件变量控制交错，不以 sleep 猜测调度。54 的 `scripts/build.bat` 退出 0，无编译／链接警告（`54-native-build.log`），Release 宿主已生成；新回归与离线复验待运行。
- 54 离线 86 张图片及确认重入、分体透明度检查通过（`54-render.log`）。定向回归 **2/3 通过**：共享设备／媒体取消和窗口／托盘／合成器通过；`widget_author_preview_cli` 继续到资源页时失败，旧最小高度 320 DIP 与新版单行指标 CPU 面板的 314 DIP 不一致（`54-targeted2-tests.log`，17.46 秒，退出 1，无警告，JUnit `4dd9d21e904c4a48aececd09b97b1ace`）。初次命令的外层 Shell 引号失败、未执行测试（`54-targeted-tests.log`），保留日志但不计为测试结果。下一调整仅收敛资源页外部预览约束，不改宿主。
- **54 最终自动证据**：宿主实现 `d4cd98a5`；仅调整外部预览测试的紧凑高度及曲线取样区后，`scripts/test.bat full` **120/120 通过**、128.35 秒、退出 0，无编译／链接警告（`54-full-tests.log`，JUnit `8726600ec59048669be8af2ea53b02b7`）。标准构建仍是 `54-native-build.log`；测试调整没有改变宿主运行源码。曲线检查排除数值卡文字，并保留冻结曲线反例；预览约束仍检查尺寸上限、留白、居中、圆角及超出画布反例。
- 显式 `scripts/test.bat name tray_live_integration` **1/1 通过且未跳过**、0.52 秒、退出 0，无编译／链接警告（`54-tray-live.log`，JUnit `ee9e68d548b34eeea899814666cc0bb1`）。生产 Service／Explorer Hook 与自有隐藏进程图标验证重报、像素／提示修改、隐藏恢复、`Shell_NotifyIconGetRect`、v4 鼠标和键盘回调、旧版右键、删除和连接释放／重连无重复。原始逐项输出保留于 `54-tray-live-output.log`。未操作第三方菜单；测试图标／连接已清理，安全固定加载的诊断 DLL 留至 Explorer 退出。
- 最终离线目录为 `controls-render-54/`、`tray-render-54/`、`calendar-render-54/`、`resource-render-54/`、`bar-render-54/`，均在 `.codex-probes/statusbar-implementation/` 下。86 个浅深色／DPI 状态已导出，实际查看控制中心、长列表、蓝牙关闭、日历第二历法、紧凑托盘和 GPU；玻璃依赖真实合成器，不以 PNG 冒充视觉实测；高对比度真实桌面尚待验。
- 资源记录 `54-offline-measurements.json`：10 个独立预览进程每 50 ms 读取一次自有进程计数，采样峰值私有内存 92.8–104.3 MiB、工作集 83.4–90.5 MiB、线程 83；单次导出约 0.84–1.09 秒。不是完整桌面闲置／开关面板的基准，也没有可用旧 WinUI 同场景基线，**不得据此声称解决 400 MB／卡顿或给出降低比例**。
...
- 用户在 51 号之后反馈：微信右键没有菜单、托盘图标不全、PowerToys 单击无反应、WPS／抖音菜单向屏幕上方越界；拖拽排序及外露固定不可用。原有协议／离线检查不覆盖这些真实应用，不能将对应行的“已有实现”当作验收完成。
- 弹窗滑入时毛玻璃缺失；不同弹窗需先关闭动画，再播放新弹窗打开动画。控制中心音量图标空间不足，无线／蓝牙主按钮应直接显示设备／网络名称，避免标签重复。
- 用户已明确：**除了设置，其他界面不要使用 WinUI 3**。控制中心、托盘、日历和资源面板现已迁移到共享原生自绘与输入设施，离线预览使用同一生产绘制代码。候选 53 已编译，当前只有设置子进程初始化 WinUI；旧 WinUI 弹窗源码不再进入构建。自动与实机验收仍分开记录。
- 控制中心不要求照搬 Win11，以简约、清晰、实用为准；按钮、滑条、开关、列表与滚动基础设施保持与设备服务解耦，方便后续 Lua 组件引擎复用。尚未新增公共 Lua 控件 API，编辑前仍须公告具体兼容变更。
- 资源评估先用 51 号二进制的隔离预览进行 debugger 分阶段采样（`memory-52/`）；调试暂停触发预览超时／窄画布检查，结果不能作为完整性能验收。迁移后须用相同、有效输入复测私有内存／线程数，不能把此前 400 MB 全归因于 WinUI。
- 最新主题要求（已澄清）：主题与材质页只保留状态栏设置链接，具体主题和场景覆盖放到状态栏设置页，与任务栏一致。**不增加右键主题选择或主题入口**，右键仍只有状态栏设置、任务管理器。设置、IPC 和每屏运行时接线已编译，对应配置回归在 53 中通过，场景视觉待实机。
- 最新媒体要求：不设媒体详情菜单；底部独立卡片压缩为单行窄条，依次是封面、标题和上一首／播放暂停／下一首。自绘布局已编译并导出浅深色生产图片，无媒体时回收空间。
- 最新提示要求：音量滚轮后当前提示应原位同步数值；状态栏和新面板已接自绘 Tooltip 和共享文字测量，已编译，实际滚轮设备和提示观感待实机验证。
- 复用审查：Lua 引擎已有控件节点、稳定身份输入与 UIA 基础；新原生草稿不能长期维护第二套同类规则。先收敛共享接口，再一次性迁移面板；设置、协议、设备服务与绘制分离，详见当前实施计划。
- 本节保留用户实测失败；后续构建和自动证据见文首，不引用 51 号全量为上述问题通过。400 MB／鼠标卡顿根因仍未确认，实际点击闪退尚无明确复验结论。

0. **2026-09-26 13:22 用户反馈点击状态栏闪退**：原始 `crash.log` 连续记录 `TranslateTransform` 激活 → `SystemPanel::Impl` 构造 → `ActivateStatusBar` 的未处理 C++ 异常。构造期创建 XAML 对象早于 `Ensure()` 初始化，离线控件预览此前先初始化运行时而遗漏该路径。51 号将对象保持为空，在 `Build()` 内创建，关闭容忍未初始化并在运行时关闭前释放；生产预览追加初始化前的真实 `SystemPanel` 构造／Hide／销毁检查。日志和二进制保留于 `click-crash-51/`，尚待构建、冷启动检查及用户原场景复验。51 首次构建期间用户启动宿主，链接因 EXE 占用失败（`51-build.log`）；不是编译成功候选。
...
### 音乐独立卡片与设备页收敛（47，编译通过，待验证）

- 用户最新要求音乐控件放在控制中心下方的独立卡片。保留一个弹窗宿主，共享关闭和焦点；两张卡片分别应用弹窗主题、圆角、玻璃区域，透明间隔不接受鼠标，滑入／滑出一起移动。没有媒体会话时收起；专门的媒体子页不再重复显示下方卡片。
- 先读取实际控件及 46 号图片，发现 Wi-Fi 底部动作竖排，蓝牙名称、电量和操作挤在同一行；本轮将 Wi-Fi 底部动作排在一行，蓝牙拆分图标、名称、状态及操作，长名称保留截断和提示。
- 现有生产预览只有单设备，不能证明长列表质量。编辑前已公告 `preview-native control-panel` 追加 `audio-many`／`wifi-many`／`bluetooth-many`／`media-empty`，旧样本保留，Lua API v2 不变。新增检查独立卡片像素／原生窗口区域的透明间隔、媒体消失时立即重新测量、长列表视口和底部操作对齐；尚待运行，不能计为通过。
- `scripts/build.bat --reload-shell` 退出 0，Release 宿主生成，无编译／链接警告（`47-build.log`）。预检宿主未运行，Explorer 持有上轮诊断临时 Hook，已按预告重载 Shell；未启动桌面宿主。生产渲染、定向／完整回归与桌面动画尚未验证。
- `b7b53aa3` 检查：浅深色各 13 个生产状态导出，卡片透明间隔、原生裁剪区域与无媒体收起通过；旧 46 号内嵌音乐图片被独立双卡片检查拒绝（`47-card-negative.log`）。定向 **3/3 通过**、57.25 秒，退出 0、无编译／链接警告（`47-tests.log`，`test-run-890f168e85e14a839916232a52e74ab1.xml`）。目检仍发现扫描图标被 Button 默认 Padding 裁掉，且音乐卡片占用了原本的主卡片高度预算，单设备声音页也须滚动；视觉不算全部通过，下一候选集中调整按钮内边距与高度分配，尚未全量。
- 48 号继续：扫描／位置图标明确内边距，蓝牙系统设置箭头缩为图标按钮；音乐占额外高度，仅在实际可用屏高不足时缩短主卡片视口。预览使用画布真实可用高度并检查扫描图标排布，尚待构建和回归。
- 48 号 `scripts/build.bat` 退出 0，无编译／链接警告（`48-build.log`）。构建结束后收到去掉单独静音设置的新反馈；先按仓库规则保存编译候选，接着合并该交互调整后统一验证，不重复执行本中间候选全量。
- 49 号合并最新反馈：移除声音页输出／麦克风的单独静音行，点击对应滑条左侧图标切换静音，控制中心首页同样使用喇叭按钮。图标、提示和无障碍动作随实际状态更新，静音不覆盖音量值；设备不可用时禁用。新增取消静音的十种语言翻译，未改变公共 Lua API。离线通过生产按钮调用验证方向、静音／恢复、音量保留及刷新不写设备；当前尚待构建和运行。
- 49 号 `scripts/build.bat` 退出 0，生成 Release 宿主，无编译／链接警告（`49-build.log`）；本轮预检无宿主或 Hook 占用，未重启 Shell。后续离线预览及回归待运行，真实设备静音操作仍待实机。
- `ec8362ae` 预览：浅色中文 96 DPI 全部 13 状态通过，生产静音／恢复调用、音量保留、双方向隔离和不可用检查通过；声音页目检滑条完整且无多余静音行。深色英文 144 DPI 前 9 状态通过，`audio-many` 因最终输出超过可用画布高度被拒绝（`49-render.log`，退出 1）；不将单主题通过当作全部验收，下一轮让最终控件测量参与高度约束。
- 50 号：主卡片视口先预算，再根据两张卡片的实际测量扣除超出量，纳入主题边框、边距及 XAML 布局取整；不放宽画布越界断言。保留 49 号原始失败作为回归输入，待构建和同输入复验。
...
| 主题位置（最新澄清） | 待迁移：状态栏设置页编辑主题和场景覆盖，主题与材质页保留跳转链接，与任务栏一致 | 跟随全局／独立预设／自定义、按屏场景、IPC 与保存恢复；不增加右键主题入口 |
...
| 音乐控件作为控制中心下方独立窄条 | 原单独卡片已有历史验证；最新原生草稿改为封面、标题、三个按钮同一行，尚未编译 | 无详情菜单；透明间隔、无媒体不占位、长标题省略、正确会话与设备能力、空间受限和统一动画 |
...
- 技术范围已被后续明确指令更新：除设置外都使用原生自绘，四类状态栏弹窗及提示纳入迁移；已有原生界面不重写。当前生产基线仍为旧实现，新草稿尚未编译；以当前实施计划为准。
- 共享设备服务已接入原生界面；六个 Lua 数据主题由 `ccfdd996` 引入（API v2、feature 门控）。十九个新增控制任务、独立写权限和可信手势／宿主确认尚未完成。
- AMD GPU 90%：问题机器原始样本仍未取得。已有拓扑缓存、独立显存有效性、PDH 缓冲复用、按需 gpu-diagnostics 和 39 号 Lua 可选详情能力；NVIDIA／Intel 证据不是 AMD 对照。系统监控组件的适配器选择仍待完成，不得声称 AMD 已校准。
- 最近完整通过基线为 51 号源码的 120/120，证据见上方记录；用户随后提出的托盘／动画失败已经单独记录。当前未编译编辑不引用旧结果为通过；下一稳定输入统一验证，不逐控件跑全量。
