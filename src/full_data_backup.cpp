...
OperationResult FullDataBackupManager::CreateAndQueueReset(
    const CancellationContext& cancellation)
{
    OperationResult result;
    // ApplyPending exchanges exactly stateRoot/data. Never queue a reset for
    // an embedding override that would replace a different live directory.
    if (!IsContainedPath(activeData_, stateRoot_ / L"data") ||
        !IsContainedPath(stateRoot_ / L"data", activeData_))
    {
        result.error = "reset data directory does not match its startup state root";
        return result;
    }
    const auto migrationRoot = stateRoot_ / L"TempState" / L"PortableMigration";
    std::error_code error;
    if (std::filesystem::exists(migrationRoot / L"pending.txt", error) || error)
    {
        result.error = "a data replacement is already pending or cannot be inspected";
        return result;
    }

    // Publishing the safety backup is harmless and must not consume the
    // reset's one cancellation gate. That gate belongs to Queue() below.
    result = Create({cancellation.stopToken, {}});
    if (!result.ok)
        return result;
    result.ok = false;
    BackupManifest manifest;
    if (!ValidateBackupDirectory(result.backup.root, manifest, result.error,
            cancellation, result.cancelled))
        return result;

    const auto token = Utf8ToWide(TimestampToken());
    const auto staging = migrationRoot / (L"staging-" + token);
    const auto discardStaging = [&] {
        std::error_code cleanupError;
        RemoveTree(staging, cleanupError);
    };
    // Copy the live tree rather than the export-filtered backup: reset must
    // retain unrelated local state, including the machine-bound entitlement.
    const auto copied = migration::CopyDataTree(activeData_, staging, cancellation);
    if (!copied.ok)
    {
        result.cancelled = copied.cancelled;
        result.error = copied.error;
        discardStaging();
        return result;
    }

    // Only these application-owned files and widget storage are reset. Their
    // recovery/temporary companions must not resurrect an old layout or data.
    constexpr std::wstring_view resetFiles[] = {
        L"SnowDesktop.layout.json", L"SnowDesktop.storage.json",
        L"SnowDesktop.widget-file-handles.json",
        L"SnowDesktop.widget-notifications.json", L"SnowDesktop.calendar.json"};
    std::vector<std::filesystem::path> removePaths;
    for (std::filesystem::directory_iterator entry(staging, error), end;
         !error && entry != end; entry.increment(error))
    {
        const auto name = entry->path().filename().wstring();
        for (const auto file : resetFiles)
        {
            if (_wcsicmp(name.c_str(), std::wstring(file).c_str()) == 0 ||
                (name.size() > file.size() && name[file.size()] == L'.' &&
                    _wcsnicmp(name.c_str(), file.data(), file.size()) == 0))
            {
                removePaths.push_back(entry->path());
                break;
            }
        }
    }
    removePaths.push_back(staging / L"widgets" / L"storage");
    for (const auto& path : removePaths)
    {
        if (error || StopRequested(cancellation, result.cancelled, result.error))
            break;
        // All paths originate from the private, reparse-free staging copy.
        if (!IsContainedPath(path, staging) || path == staging)
        {
            error = std::make_error_code(std::errc::permission_denied);
            break;
        }
        RemoveTree(path, error);
    }
    if (error || result.cancelled)
    {
        if (error)
            result.error = "cannot clear staged layout and widget data: " + error.message();
        discardStaging();
        return result;
    }
    // Empty primary documents also prevent legacy-path migration from
    // importing old files. Empty layout dimensions trigger normal DPI-aware
    // page initialization, enumeration and placement on the next startup.
    if (!WriteTextFile(staging / L"SnowDesktop.layout.json", "{}\n", result.error) ||
        !WriteTextFile(staging / L"SnowDesktop.storage.json", "{}\n", result.error) ||
        !WriteTextFile(staging / L"SnowDesktop.calendar.json",
            "{\"schemaVersion\":1,\"events\":[]}\n", result.error) ||
        !WriteTextFile(staging / L"SnowDesktop.widget-notifications.json",
            "{\"schemaVersion\":1,\"entries\":[]}\n", result.error) ||
        !WriteTextFile(staging / L"SnowDesktop.widget-file-handles.json",
            "{\"schemaVersion\":1,\"entries\":[]}\n", result.error))
    {
        discardStaging();
        return result;
    }
    if (!migration::Queue(stateRoot_, token, result.error,
            cancellation, &result.cancelled))
    {
        discardStaging();
        return result;
