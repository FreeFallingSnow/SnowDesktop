...
struct Reply { bool ok = false; std::vector<Entry> entries; };
...
template<> struct Fields<shell_extensions::Request> {
    template<class T> static auto Tie(T& v) { return std::tie(v.paths,v.background,v.catalogueOnly,v.extended,v.providers); }
};
template<> struct Fields<shell_extensions::Entry> {
    template<class T> static auto Tie(T& v) { return std::tie(v.provider,v.key,v.label,v.token,v.enabled,v.checked,v.separator,v.native,v.children,v.width,v.height,v.pixels); }
};
template<> struct Fields<shell_extensions::Reply> {
    template<class T> static auto Tie(T& v) { return std::tie(v.ok,v.entries); }
};
}
