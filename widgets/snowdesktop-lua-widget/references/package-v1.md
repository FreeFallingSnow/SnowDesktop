...
`permissions` 中的能力是组件启动所必需的；可降级功能可放入
`optionalPermissions`。用户只授予必需权限时组件仍可启动，但相应可选 API
保持不可用；同一权限不得同时出现在两个数组中。

系统状态读取已拆分为 `system.performance.read`（CPU、内存、GPU）、
`system.power.read`（电源、电池）和 `system.network.read`（网络状态、传输速率）。
旧的 `system.read` 通配权限不再通过包校验；可选能力应先用
`widget.hasPermission(name)` 判断是否已授予。

校验器同时保留了细粒度 v2 权限名称，供迁移工具和宿主授权模型使用；API v1
并未因此开放对应的存储、显示器、音频分析、v2 网络、剪贴板、应用启动或用户选择
文件 API。v1 组件不得把“清单可通过校验”当作对应 API 已存在。
...
snowwidget preview widgets\my-widget my-widget-preview.png --locale zh-CN --theme dark --data-state ready
