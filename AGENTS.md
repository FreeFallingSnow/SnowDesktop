...
- 清理已有分支前，先核对远端地址、工作区状态和提交包含关系，确保需要保留的提交已进入 `main`；再删除其他本地和远端分支。不得为日常工作留下 `release/*`、`codex/*` 等分支。
- 推送前读取远端 `main` 的最新状态。优先正常推送；确需改写远端历史时，仅在确认待保留提交均已纳入、不会覆盖他人新提交后使用 `--force-with-lease`，不得盲目 `--force`。
- 网站仓库推送 Git `main` 不等于线上官网部署；网站发布仍按 `website/AGENTS.md` 执行备份、构建、部署和线上核对。商店资产仓库推送 `main` 不等于 Steamworks 文案导入。
...
- 以用户实际收到的上一次正式构建为对比起点，按最终发布内容归并多次开发提交。文案只写用户
  能直接感知的新功能，以及上一次正式构建中存在、这次已验证修复的缺陷；同一版本号内多次
  正式更新也按实际构建比较，不按版本号推断。
- 不写实现技术、内部流程、构建和测试过程、日志、提交或分支信息；不写开发和测试过程中出现、
  在正式发布前已消除的临时问题及其修复。普通调校和未确认的修复也不凑作更新条目。
- 从用户可以新增执行的操作、实际看到的变化，或不再遇到的既有问题来表述。草稿、正式说明
  和翻译均遵守同一范围；没有实际验收依据的缺陷不得写成“已修复”。
...
- Agent 使用 SteamCMD 登录账号执行上传、预览或远端版本查询后，必须在整批操作结束时恢复
  原先正在运行的桌面 Steam 客户端。用户已授权自动重启，无需再次确认；执行前简短告知即可。
  同账号登录可能使客户端收到 `Session Replaced` 并停止自动重连，SteamCMD 的 `+quit` 不会
  自动恢复客户端原会话。
- 首次调用 SteamCMD 前记录桌面 Steam 是否运行及其实际可执行文件路径。把所有需要登录的
  上传和查询集中完成，等待本任务启动的 SteamCMD 全部退出后再统一重启一次；成功、失败或
  取消后的收尾均须处理恢复，不得刚恢复客户端又为补充查询登录 SteamCMD。
- 客户端原先正在运行时，使用记录的 Steam 可执行文件通过 `-shutdown` 请求正常退出，有限
  等待退出后再以 `-silent` 启动；通过 `Start-Process` 启动时使用 `-WindowStyle Hidden`。
  不得强杀整个进程树或连带终止游戏。原先未运行的客户端不主动启动；用户后续明确要求保持
  关闭时遵从该要求。
- 恢复后只读检查客户端进程和本次重启之后的连接日志，不得再登录 SteamCMD 验证恢复结果。
  分别报告“客户端已重启”和“已重新在线”；只有新日志确认登录成功才能声称恢复在线。
  正常退出受阻、重启失败或需要 Steam Guard／人工登录时，如实说明当前状态和所需操作，
  不得将已发出启动命令视为恢复成功。
...
- 本轮标准构建、定向编译或测试构建中出现的**所有编译警告和链接警告**，必须立即定位并在
  当前任务中修复，不限编译器、警告编号、模块或来源。
  不得因警告是历史遗留、不是本次改动引入，或诊断位置位于第三方／生成头文件中就搁置；
  警告来源应如实记录，用于追溯，不能作为不处理的理由。
- 应从源码、头文件包含顺序、兼容边界或构建配置修正根因；不得以降低警告级别、关闭警告、
  扩大警告抑制范围或仅在交付中标注“既有警告”代替修复。生成文件中的警告应修正其生成输入
  或项目接入方式，不得直接修改生成文件来掩盖问题。
- 修正后必须重新运行相应编译；最终交付仍须完成标准 `scripts/build.bat`，核对完整日志中的
  目标警告已消失，并检查是否产生其他编译或链接警告。按改动风险完成所需回归，不得把
  “编译成功但警告尚未处理”视为本轮构建验证已经完成。
### 测试入口、选择与检查点
...
- `.codex-remote-attachments/` 保存用户提供的临时附件，不得提交；整理工作区时保留原文件，不得当作无用文档删除。
- `docs/` 优先保存持续有效的规范、设计说明、操作指南和未关闭问题清单。一次性排查、逐轮尝试和带时间戳的验收流水优先保存在对应提交、版本日志或忽略的临时目录，不持续堆积为独立文档；清理已提交记录前须确认没有被规范引用或承载尚未迁移的未关闭问题。
