...
- 核心拖放验证覆盖来源/目标、载荷、效果、同步/异步、取消和失败；支持与拒绝关系均须登记。已知缺陷和高风险组合
...
- 评估中发现的非测试程序问题单独记录在 GitHub Issues，注明稳定编号、生产位置、触发条件、影响、
  证据、验证状态、关联测试及关闭条件。区分已复现问题、代码推导风险和待确认项；
...
- 除忽略的生成输出外，`docs/` 不提交文档。持续规则写入 `AGENTS.md` 或对应模块的说明文件；一次性排查与验收证据记录在提交、版本日志或忽略的临时目录，未关闭产品问题记录在 GitHub Issues。
