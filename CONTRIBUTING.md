...
- [Steam](https://store.steampowered.com/app/5080330/SnowDesktop/) 测试版：参与新功能体验与问题验证；加入方式可通过 QQ 群咨询。

以下开发规范用于维护者当前的开发工作，并供未来开放代码贡献时参考；不代表现阶段接受外部代码 PR。
...
1. 从官方仓库当前开发版本的 `release/vA.B.C.D` 分支创建工作分支。外部代码贡献暂停期间，请通过上述交流渠道反馈。
...
你保留对自己原创贡献的版权。贡献按目标位置适用的现有许可证提供，提交贡献不表示转让版权。
SnowDesktop 可在遵守相应许可证条款的前提下使用这些贡献，并进行商业使用和收费发行。
第三方内容仍须遵守其原有许可证和声明。

## 第三方内容与来源说明

- 仅提交自己创作或有权提供的内容。若雇主、客户或其他权利人拥有相关权利，先取得足以覆盖所提交许可的授权。
- 引用第三方代码、图片、字体、数据或其他材料时，注明准确的上游地址、版本或提交、许可证和修改内容；保留版权与许可声明，并按需更新 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。不要把第三方内容标成自己的原创贡献。
- 不得把 GPL 核心或 TranslucentTB 衍生代码复制进 `steam_bridge/`。两者通过有文档说明的命令行、JSON、命名管道等进程边界通信；共同使用的协议定义必须原创并明确采用宽松许可证。这项架构规则不替代对具体组合的许可审查。
- Steamworks SDK 及其头文件、库、工具和可再分发文件不得提交到仓库。构建启用 Steam 的桥接程序时配置外部 SDK 路径。
- 使用 AI 辅助生成贡献时，仍需检查来源、许可兼容性、正确性及敏感信息；涉及已知第三方材料时照常披露其来源。
