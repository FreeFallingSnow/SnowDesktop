...
void PrintUsage()
{
    std::cout
        ...
        << "  snowwidget preview-native <collection|collection-group|file-group|file-categories|folder-mapping|calendar-panel|control-panel|tray-panel|all> <output-directory>"
           " [--dpi N] [--locale CODE]"
           " [--appearance dark|light|glass-dark|glass-light|acrylic-dark|acrylic-light]"
           " [--background image-file]"
        ...
        if (argc < 4 ||
            (component != L"collection" &&
             ...
             component != L"file-categories" &&
              component != L"folder-mapping" && component != L"calendar-panel" && component != L"control-panel" && component != L"tray-panel" && component != L"all"))
        {
