...
该工具在耦合运行时采集主进程的桌面、Dock、Lua 实例及后端诊断数据，默认关闭。
...
设置界面现已迁入[按需退出的独立进程](settings_process.md)。本入口仅连接主进程，
不会自动采集或合并设置子进程的打开、导航、快照应用和 XAML 布局/呈现阶段。
设置内存/CPU/GPU 评估需要分别采样主进程和当次设置 PID；XAML 深入分析需使用
对应子进程的 WPR/WPA 跟踪。关闭后 PID 消失表示进程退出，缺失计数器不等于 0% GPU。
