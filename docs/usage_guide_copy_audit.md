...
用户指出原审查对网格和图标说明的保留结论不充分，现撤回这两项结论。网格行列决定页面的整体布局尺度，减少格数会放大单元及图标、组件，不能解释为增大间距。图标页调整图标和文字显示，组件与布局页调整共享布局间距及组件圆角、标题栏、分类标签、Lua 内容行高。

本次改写网格和图标指南，新增“调整组件与布局”指南，总数为 32 条；十种语言同步。布局间距控件移至组件与布局现有卡片，旧 `desktop.spacing` / `desktop.iconSpacing` 路由及搜索结果同步指向新页面，存储字段和更新流程不变。下限由 50% 改为 10%，控件和实际网格计算共用范围，右键菜单增加 10% 和 25% 选项；有效间距仍受可用空间限制。

Lua 指南第三步已按用户要求删去网络或文件夹配置的冗余后半句。本报告前面的保留结论仅记录首轮审查，不代表后续文案未经调整。

## 发布前术语统一（2026-09-17）

本轮覆盖 32 条指南的十种语言，修改语言资源中的 158 个值，保持键集合、格式占位符、设置路由和公共 API 不变。

- 繁体中文统一集合組、跳到、釘選並移至 Dock、喚起与快速導覽；简体中文搜索目录使用与实际设置卡片一致的“快捷导航”；日语统一クイックナビゲーション。
- 两种西语使用与菜单一致的 Asignación；固定到 Dock 的操作直接引用各自语言的菜单名称。同步拉美西语的桌面文件名称并修正 Dock 说明中的冠词、单复数错误。
- 英文使用 Pages & grid 和实际 Dock overlap 开关名；法语映射组件名称使用 Mappage de dossiers；韩语键盘翻页说明明确为拖动图标，检查更换术语后的助词。
- 十种语言的 Dock 常驻及空间说明均引用对应的完整控件名称。英语、德语、日语、韩语和葡语的指南及其直接引用的添加菜单、导航、布局与组件管理文案采用 Component / Komponente / コンポーネント / 구성 요소 / Componente，避免单独修改指南后指向不存在的菜单。
- 其余历史界面和开发资料中的 Widget 不是本次全库重命名对象；内部 widget 标识、文件名、Skill 名称及组件 API 保持原样。这里的统一范围是指南及对应操作入口。

文案级复核：十种语言键集合保持不变，120 处指南引用的设置/菜单名称与实际翻译值一致，并检查本轮已知误译、错误术语和语法片段不再出现。此核对为临时审查脚本，不以机械断言代替语言审读。

实际构建与测试结果见本轮提交正文。长文案换行、裁切和十种语言的桌面视觉仍待用户实机；文案静态一致性不代表这些视觉场景已经验收。
