...
## 2026-09-21：卡片视图、来源核对与滚动边界候选

候选按仓库规则分别保存为 `3c586061`、`fef59259`、`394ac065`；诊断测试提交为
`5b26caf2`。开发分支及应用版本不变，私有设置 IPC 为 21，组件公共 API 不变。

- 扩展管理切换为按对象、按应用、按文件类型三种卡片视图；空白处提供前两种。
  卡片顶部总开关写入原始成员身份，跨后缀共享动作同步，保留位置例外。
  检查文件／文件夹／桌面及刷新放入“更多”；补齐全部十种语言和设置搜索。
- 真实 Shell 菜单继续决定显示项目；隔离来源查询只补充应用资料，不能发布额外项目或执行命令。
  无规范命令名时要求图像和完整层级等证据，不按标题猜测；来源冲突撤下归属。
  同一动作不会因样本路径重复检查，失败来源本轮跳过，刷新或注册变化后允许重查。
- 不可滚动侧同时撤下提示和占位；绘制、点击、滚动和键盘导航使用同一视口。
  初版顶部回归在成功点击后同步发送 Escape，覆盖了返回命令；调整为排队取消后，
  顶部与底部边界在 Win10 深浅色、100%／125%／150%／200% 缩放通过独立菜单交互检查。

本轮证据均位于 `.codex-probes/menu-attribution/`，未把失败算作通过：

| 检查 | 实际结果 |
| --- | --- |
| 定向六项 | `targeted-delivery.log`，6/6 通过，退出码 0，编译 15.74 秒，CTest 20.02 秒；报告 `test-run-2033cdc95b9f4bf5b5ddee0a80cd7547.xml` |
| 首轮完整 `scripts/test.bat` | `full.log`，118/119，退出码 1，CTest 95.01 秒；报告 `test-run-793ddab68cf84785970ec89dad6c5450.xml` |
| 带计时单项诊断 | `shell-failure-diagnostic.log`，通过；四种真实位置均返回，文件／文件夹查询为 188／343 毫秒 |
| 带诊断的完整顺序检查 | `full-order-diagnostic.log`，115/119，退出码 1，CTest 238.12 秒；报告 `test-run-7ea021a4f2cc450b86db2941cd9d6ece.xml` |
| 标准构建 | 首轮 `scripts/build.bat --reload-shell` 及最终标准构建退出码均为 0，日志 `build-cards.log`、`build-delivery-native.log`、`build-delivery-reload.log` |

首轮完整测试的 `shell_context_menu_invoke` 在文件解析、文件夹绑定阶段超时。
单项带计时未复现；完整顺序诊断再次在文件夹解析阶段超时，同时
`deployment_packaging_contract`、`steam_local_deploy_contract`、`test_selection`
达到各自 60 秒上限。查询上限保持八秒，没有删除失败断言、放宽超时或增加无条件重试。
未发现相应 Windows 应用崩溃记录；出现过 PowerShell 预检启动后延迟执行，最终原生
cmd 下的标准构建完成。这些现象的因果关系尚未确定，完整回归未全部通过，不能视为稳定性验收。

隔离故障副本正常编译并以退出码 1 命中明确断言：恢复固定顶部占位后，首行边界点击失败；
恢复按路径核对来源后，相同动作重复查询。见 `negative-build-final.log`、
`negative-scroll.log`、`negative-source.log`。未覆盖生产源码，也未以编译失败或超时作为检出证据。

本机空临时缓存只读诊断：117 个原始根项，合并后 93 个文件／文件夹项目；19 个来源应用，
27 行尚未识别（此前相同入口为 75 行尚未识别）。首版来源扫描在 180 秒时仍未完成，
去重后单次后台完成为 36.281 秒；30 次内存目录读取 P95 为 0.2277 毫秒。
首个空目录读取为 0.0437 毫秒，不等于首次项目呈现；以上不包含 WinUI 创建和窗口绘制，
单次冷样本不能证明所有机器上的性能改善。原始记录为 `baseline.log`、`real-first.log`、
`real-dedup.log`。产品名称可能来自模块资料或 DLL 文件名，不保证都是应用商店展示名。

996 个源码、测试、语言、资源及构建输入在本轮检查前后哈希一致，见
`final-inputs.json`、`final-verification.json`。Release、MSBuild 18.5.4、SDK 10.0.26100.0。
最终宿主 SHA256：`789830223E7771320925577C9A4964ECC9B6789C959CDF606C9DBBC488115AA3`。
首轮编译出现既有 WinUI 生成头 GetCurrentTime C4002；最终增量构建没有新编译警告。
重载脚本的非交互 timeout 提示仍存在，构建最终退出码为 0。

设置页浅深色、窄窗口、卡片总开关反馈、后台补齐时的展开状态，以及原截图桌面滚动场景
仍待用户实机验收；27 个未识别来源和完整回归中的偶发／脚本超时仍是已知限制。
本轮不创建实机 `verify`，也不把单项诊断成功代替全量成功。

## 2026-09-21：设置卡片对齐与刷新提示候选

本次仅调整 WinUI 右键管理页及十种语言：卡片总开关靠右，移除额外的 48 DIP
右侧留白；条目展开箭头的前间距由 20 DIP 调为 4 DIP，保留原生 32 DIP 点击区域。
“扩展菜单项”标题旁增加 16 DIP 进度环和“正在刷新…”；由宿主返回的
`scanning` / `menu.pending` 驱动，有列表时也显示，不以分批创建缓存行作为忙碌依据。
完成、查询异常和页面停用时停止提示，异常后不会因搜索重新使用过期的忙碌状态。

调用范围限于 `ContextMenuPagePresenter` 的呈现及原有查询回调；目录扫描、调度、
缓存格式、配置写入、私有 IPC 和组件公共 API 均未改动。按普通交互调校候选执行定向
检查，没有为间距增加源码字符串测试，也没有重跑全量。此前完整回归的超时仍未关闭。

- `scripts/test.bat name "^(localization_contract|settings_controller|settings_search_index)$"`：
  3/3 通过，退出码 0；配置 2.05 秒、编译 2.27 秒、CTest 1.91 秒；报告
  `test-run-c49a847f384b436d8133ba2dfca15faa.xml`。
- `scripts/build.bat --reload-shell`：标准 Release 构建通过，退出码 0，已生成
  `.build/Release/SnowDesktop.exe`；预检发现宿主及 Explorer Hook 占用，已提前说明重载副作用。
- 构建未报告编译器警告；重载脚本仍出现已知的非交互 `timeout` 输入重定向提示，
  后续构建及输出整理正常完成。日志保存在 `.codex-probes/menu-settings-layout/`。

实机待验：浅深色、不同 DPI 与窄窗口下的总开关对齐和箭头距离；首次发现／手动刷新／
已有列表后台补齐时显示提示，缓存静止、查询完成／失败、离开页面后停止；切换分类、
搜索及重新进入页面时状态一致。自动化未执行这些 WinUI 场景，按 `try` 保存；本次不
声称查询耗时或缓存稳定性已经改善。

## 2026-09-21：标签同行对齐与无图标子菜单候选

右键管理页的分类与视图标签合并到一行，分类靠左、视图靠右，搜索仍在下一行。
自绘子菜单逐级判断普通行：没有字形、图片或勾选的一级收起图标栏及图文间隔；
禁用项中的图标／勾选仍保留该级对齐，分隔符和下一级图标不影响当前级。
文字起点与宽度测量共用留白计算，快捷键间距、最小菜单宽度不变。根菜单、搜索框、
行内分页控件使用原尺寸。独立预览新增无图标、混合图标及下一层含图标的示例。

影响映射：`CalculateLayout` 为每个弹窗保存普通行尺寸，`MeasureItem` 与 `DrawItem`
消费相同尺寸；原有正宽图标栏路径保持等价，组件预览共享渲染依赖。设置改动仅重新
摆放现有两个 SelectorBar，事件和导航目标不变。无公共 API、缓存、配置或调度改动。
按普通交互调校运行受影响定向回归，本次未重跑全量；此前全量超时仍未关闭。

| 本次命令 | 结果 |
| --- | --- |
| `scripts/test.bat name "^(settings_controller\|settings_search_index)$"` | 2/2，退出码 0，编译 1.69 秒，CTest 1.19 秒 |
| `scripts/test.bat name "^(menu_icon_render\|modern_menu_interaction\|component_preview)$"` | 3/3，退出码 0，编译 19.12 秒，CTest 5.90 秒 |
| `cmake --build .build --config Release --target SnowDesktopModernMenuPreview` | 独立预览目标编译通过，退出码 0 |
| `scripts/build.bat --reload-shell` | 标准 Release 构建通过，退出码 0，已生成宿主；占用预检及重载提醒已完成 |

CTest 报告为 `test-run-057a248afa9440658e969bd556d65d5c.xml` 和
`test-run-3ab68785301e4069ae672efb9bf20439.xml`。新增回归使用实际绘制及测试专用隔离
桌面的真实菜单，覆盖 100%／125%／150%／200%、不透明深浅色及 Win10 深浅色、
快捷键测量、字形／图片／勾选、禁用图标、父子级独立判断和命令返回。
隔离旧实现正常编译后，渲染与交互两个对照均以退出码 1 命中新增留白／宽度断言；
没有覆盖生产源码，没有把编译失败或超时计为检出。日志位于
`.codex-probes/menu-selector-alignment/`，`negative-render.log`、`negative-interaction.log`
为负向证据。

标准构建出现既有 WinUI 生成头 `GetCurrentTime` C4002；无新增编译警告。
重载脚本仍有已知非交互 `timeout` 输入重定向提示，最终构建及输出整理正常完成。
设置页窄窗口和各语言下的标签排布、真实 Shell 子菜单视觉及桌面点击仍待用户实机
验收，按双语 `try` 保存，不将独立菜单回归作为这些实机场景的通过结论。

## 2026-09-21：有效开关、缓存投影与按需查询候选

候选 `b19b8846`、`2e40effc`、`a7a59006` 保持 `release/v1.0.7.0` 和版本 `1.0.7.0`。
当前场景没有有效开启项时，普通查询和预热跳过；混合选择的隐藏例外优先，已知注册范围
及系统禁用状态参与判断，未可靠关联的动态来源保持保守查询。关闭开关撤下未启动的普通
任务，但保留明确的设置检查订阅；已启动的有效查询允许完成并缓存。

显示使用宿主当前开关投影原始快照，首次动态补齐也读取最新规则。主开关及卡片批量操作
重置本分类成员的位置例外，避免迁移出的显示例外盖过用户刚提交的隐藏操作；另一分类
不变，用户可以随后再添加位置例外。对现有配置的只读检查观察到 63 条通用规则、13 条
位置例外，其中 6 处例外与通用值相反；本次没有直接改写该配置。

有效快照不再十秒后自动重查；目标属性检查仍在后台，相关注册变化和过期使快照失效，
主动刷新及点击执行仍重新查询。空闲服务等待请求和注册事件，活动查询／扫描使用有界
轮询；注册事件改为手动重置，避免等待消耗通知后漏掉重新订阅。目录、IPC 和缓存文件
格式、组件公共 API 均未改变。Windows 冷查询仍是聚合菜单，不能保证只激活已选处理器。

调用与风险映射：`Configure` 计算有效开关并取消排队任务；`Queue` 和工作线程分别进行
内存范围判断及实际对象判断；`MenuDisplay` 投影缓存；`Presentation` 消费投影；执行完成
时仍通过最新规则和新会话令牌校验。管理开关写入经 `SetManagementCommon` 统一处理。
新增测试替换的仅是辅助进程边界，调度、订阅合并、配置、磁盘恢复和菜单转换保持真实。

- 第一轮定向编译通过，测试 3/4；旧批量测试断言仍要求保留位置例外，与新的主开关语义
  冲突。随后更新该行为预期，并保留另一分类不变的断言，没有删除失败用例。
- 第一轮 `settings_controller_tests.cpp:302` 出现 C4456，重命名遮蔽变量后重新编译，
  第二轮没有编译或链接警告。
- 第二轮 `scripts/test.bat name "^(shell_context_menu_invoke|settings_controller|general_settings|modern_menu_interaction)$"`
  编译通过，4/4 测试通过，退出码 0；配置 1.71 秒、编译 20.78 秒、CTest 20.90 秒。
  报告 `test-run-b1bb3a63351e451c863939e4ed526f78.xml`。
- 新回归覆盖旧磁盘快照的 30 次连续打开不启动查询、当前开关替换捕获的旧设置、全部隐藏、
  旧 opt-in 被明确隐藏覆盖、位置及文件类型范围、系统禁用、首次动态补齐中途关闭、混合
  选择、对象变化失效、排队取消与主动检查保留。已有失败保留快照及点击重查测试继续通过。
- 隔离目录中当前实现 `control` 退出码 0；`projection` 恢复旧设置投影、`override` 保留
  冲突位置例外、`disabled` 忽略有效开关，三者正常编译后均由相应断言退出 1。
  没有覆盖生产源码，也没有把编译错误、超时或重试作为检出证据。

最终跟进候选另外保护两条主动检查路径：新预热不能取消共享给设置页的旧预热；全部隐藏
时，主动检查发现对象属性变化后仍可重查。首屏投影保留命中及耗时日志。
最终定向命令与上轮相同，4/4 通过，退出码 0；配置 1.92 秒、编译 13.09 秒、CTest
18.72 秒。报告 `test-run-3e8f9e0402cb4bb7a7cfcf759da154ba.xml`。最终隔离源码重新编译，
当前实现仍退出 0，前三项对照仍分别退出 1；新增 `subscriber` 恢复取消共享预热后，
由“changing selection cannot cancel a prewarm shared with explicit inspection”断言退出 1。

| 最终候选本次命令 | 实际结果 |
| --- | --- |
| `scripts/test.bat` | 自动集合 119/120，脚本退出 1、CTest 退出 8；配置 1.58 秒、编译 26.84 秒、CTest 98.06 秒 |
| `scripts/build.bat --reload-shell` | 标准 Release 构建通过，退出 0，16:15:52–16:16:23；已生成宿主及运行目录 |

全量唯一失败为 `slot_runtime_contract` 的“external file creation wakes the mapped-folder
subscription”，该测试执行 7.40 秒，未观察到预期的映射目录通知。其通知实现及夹具来自
`83f6f855`，本轮未修改，也未链接 `shell_extension_service.cpp`；没有用无条件重跑掩盖
失败，没有把 119/120 写成全量通过。当前原因仍待该独立目录通知路径调查。完整报告为
`test-run-57eda22703df42febb420b65b6649e25.xml`。右键查询测试在全量中通过（12.15 秒）；
手动 Shell 文件操作诊断未运行。

最终标准构建前核对进程及 Explorer Hook 占用，并提前说明重载副作用。最终定向、全量、
隔离对照及标准构建日志均无编译／链接警告；重载脚本仍有非交互 `timeout` 输入重定向
提示，后续构建与产物整理成功。初轮 C4456 已修正并重编译，不作为未处理警告遗留。

日志与隔离源码位于 `.codex-probes/menu-query-policy/`。`inputs.json` 和
`final-verification.json` 核对 1,050 个源码、测试、资源及构建输入未变化，绑定代码提交
`a7a59006d20dfc0338784fdb1b31c3d5498a62eb`、MSBuild 18.5.4、Windows SDK 10.0.26100.0、Release。
最终 `.build/Release/SnowDesktop.exe` 为 16,670,208 字节，SHA-256：
`8094793b1e43f905a582342aecbc3eda7cabc1c9d5ec4c61668ff67fe705f504`。

桌面实际点击、CPU 峰值及第三方动态内容时效尚未验收，不把减少受控查询次数写成已测量
的 CPU 降幅。无对象／注册变更通知的第三方动态内容可能保留至快照过期；可手动刷新，
点击前仍查询新会话并核对当前命令。
