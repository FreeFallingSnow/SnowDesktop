...
日期：2026-09-14。首轮基于 `1214624c`，保存在 `45b70297`；本轮补充映射互拖、单项组还原及 Dock 桌面文件支持。桌面宿主交互尚待用户实机验证。
...
7. 新建集合组及三种组合的文件组，分别把其中一个标签拖回桌面、移入另一个组、从文件组拖入 Dock 或删除，核对剩余项自动还原为原类型。分别留下集合、桌面文件和映射组件；核对路径、文件、名称、显示设置和尺寸，刷新与重启后不恢复组。测试组附近拥挤、原尺寸大于组时不会覆盖其他组件；手动创建的单项组保持原状。
