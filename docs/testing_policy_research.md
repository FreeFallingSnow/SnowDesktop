...
本报告为[测试规范草案](testing_policy_draft.md)提供依据，不替代当前 [AGENTS.md](../AGENTS.md)，不表示已实现新的测试选择或缓存机制。调研期间未安装候选 Skill，未修改现有规则、测试脚本或生产代码。
...
1. 先采用[草案](testing_policy_draft.md)中的风险与阶段选择表、结果复用条件和结论用语，统一修改 AGENTS 中重复的全量触发规则。
...
本次交付只有调研报告与规则草案。旧测试清单和执行机制仍在，尚不能宣称已完成测试体系重构或已获得性能提升。
