...
1. 2026-09-11 后续用户确认 DND-01、DND-02 已修复，按原场景实机反馈关闭；DND-03、DND-04 后续在 `f91bb2c8`、`8300848d` 中调整生产代码，独立落点/集合模型提交和真实部分失败清理回归通过，仍按问题清单等待宿主实机验收。此次关闭依据是用户反馈，不是矩阵规则或全量测试；具体记录见生产问题清单。
