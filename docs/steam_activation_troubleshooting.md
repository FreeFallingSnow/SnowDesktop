...
| 客户端版本不兼容 | 在 Steam 菜单中检查客户端更新，更新后完全退出并重新打开 Steam，再从库中启动 SnowDesktop；仍失败时提供“帮助 → 关于 Steam”中的客户端版本、构建日期及错误详情 |
...
初始化结果 `3` 表示客户端接口版本不兼容。例如 `steam_init_failed: No SteamUtils011`
表示当前客户端无法提供所需接口，应优先更新 Steam 客户端。此时尚未完成购买资格检查，
`owned=absent` 不表示未购买；应按上述步骤处理后重试，再结合新日志判断结果。
