...
view.row({
    key = "feed:" .. item.id,
    style = { backgroundColor = 0x00000000 },
    hoverStyle = { backgroundColor = 0x18FFFFFF },
    pressedStyle = { backgroundColor = 0x2AFFFFFF },
    focusStyle = { strokeColor = 0xFFFFFFFF, strokeWidth = 1 },
    transition = {
        durationMs = 120,
        properties = { "backgroundColor", "opacity", "transform" }
    },
    children = { ... }
})
```
...
- 图片解码、字体解析和 GPU 资源创建不得发生在 `view()` 或 `render()` 热路径；加载返回 pending/ready/error 状态，失败提供稳定错误码和可选 fallback。
...
当前过渡实现（2026-08-15）发布 `view.tree.core`，支持 `box/row/column/stack/text/image/button/icon/iconButton/shape/progressBar/progressRing/spacer`，以 `view.grid.uniform` 提供 1–64 个等宽列、行优先顺序和独立行列间距的基础 `grid`（不包含 track/span/虚拟化），以 `view.statusVisuals` 公开 `badge/divider/meter`，以 `view.dataSeries` 公开 `sparkline/lineChart/barChart/waveform/spectrum`，并以 `view.selectionControls` 公开受控 `toggle/checkbox`；选择控件要求显式 `checked` 与 `change` action，点击时宿主只返回 `previousChecked/checked` 建议值而不替组件持久化，checked/hover/pressed 视觉均由宿主直接绘制；meter 与数据图形要求无障碍标签，数据图形每节点最多 512 个有限样本、全树最多 4096 个并支持自动或显式 `min/max` 值域、
