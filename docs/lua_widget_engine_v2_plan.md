...
- 声明式 transition 由宿主运行；`view.transition.visual` 只允许显式列出的颜色和透明度，`view.transition.transform` 允许显式 transform 呈现过渡，`view.transition.layout` 允许稳定 key 节点的父相对位置与尺寸过渡，`view.transition.enter` 允许新增 key 从有界 opacity/transform 起点入场。exit 动画尚未开放，不能靠无条件 Lua 逐帧循环模拟。
