...
- 包资源路径在 VM 建立时一次解析；图片由 `resource.image` 在入口加载期计算内容 SHA-256，
  ...
  私有字体也必须在入口加载期创建集合，组件卸载不会清空其他实例仍共享的资源缓存。
