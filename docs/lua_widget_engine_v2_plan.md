...
- 正式切换后的宿主与作者工具只接受 schema v2/API v2；预发行 API v1 不再作为受支持输入格式。
...
- 正式宿主与作者工具只接受 API v2，不解析或运行 API v1 组件。
...
- schema v1 / API v1 包在校验阶段直接拒绝，不进入包管理或 Lua VM。
...
- 预发行 API v1 包的隐式授权不迁入 v2；作者需要直接创建新的 v2 包并按新清单首次授权。
- 已有布局若没有有效 v2 包身份，不创建组件实例或执行 Lua。
...
- 新组件必须直接声明 v2 网络权限和精确 origin，工具不得自动生成通配授权。
...
- 元素菜单固定保留“打开组件面板”入口；组件菜单不能隐藏设置、授权、诊断和移除组件的宿主管理入口，宿主管理操作必须始终可达。
...
- `snowwidget system-contract`：离线输出 `system.capabilities()` 使用的 15 项系统函数、25 个
  ...
  工具白名单或结构猜测表。
- `snowwidget lint <directory>`：先用 Lua 编译器给出稳定 `lua.syntax` 与行号，再接入同源
  API/系统能力/视图节点目录，静态检查不存在或版本
...
- `snowwidget preview <directory> <output.png> --columns ... --rows ... --dpi ...
  ...
  unavailable；accent、无障碍默认值、region、UTC 时区和输入语言同样不读取开发机状态，
  存储使用隔离覆盖层且不写持久状态。
...
- 超时、内存和指令额度；
- 嵌套宿主回调和 Lua 栈恢复；
- 非 schema/API v2 包在校验阶段拒绝；API v2 正常激活。
...
M7 切换完成后，发布运行时和作者工具必须删除 API v1 注册、解析和执行分支，不能以假设中的第三方兼容需求阻止移除。
...
- 安全包内模块、图片和包私有字体资源句柄。
- v2 LuaLS 类型和迁移指南初版。
- 当前正式加载路径和作者工具在读取入口正文前强制要求 schema/API 同为 2；
  schema/API v1 的清单解析、迁移诊断、API 注册和执行分支均已删除。正式 VM 的
...
- 冻结 Lua API v2.0 契约。
- 发布兼容承诺、弃用策略和安全响应流程。
- 11 个内置组件、正式宿主和作者工具全部只使用 API v2。
...
| 预发行自制 v1 包停止运行 | 少量早期作者需要重写组件 | v2 模板、文档和清晰校验错误；不保留不安全执行或伪迁移路径 |
...
- schema v1/API v1 包在校验阶段拒绝，正式宿主与作者工具不存在 v1 解析或 API 注册。
