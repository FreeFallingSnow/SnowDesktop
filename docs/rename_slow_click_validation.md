...
2026-09-21，用户在收到 `11831c15` 后反馈“没问题，发布到steam测试分支吧”。据此记录本轮普通单击名称及移动指针问题获得用户认可，不推断下列所有界面、缩放和异常场景已逐项实测。此前交付的 EXE 为 `8A2D29EDFEF63A6D3E1398D0C02291F0EB89801E00430C0D94B119B2CE13830E`。

发布检查保存在 `artifacts/v1.0.7.0/rename-test-20260921-11831c15/`：

- 首次 `scripts/test.bat full` 在运行文件占用预检退出，未开始编译或测试；随后 `scripts/build.bat --reload-shell` 退出码 0，30.75 秒。
- 再次 `scripts/test.bat full` 退出码 0，120/120 通过；含构建 106.71 秒，CTest 85.22 秒，默认排除 manual。报告 `.build/Testing/test-run-b10d227fe691496fa23ae468f4a86f91.xml`。无编译或链接警告。
- 1,070 个源码、测试、资源和构建输入与已验收候选的清单内容及文件集合一致。构建与完整测试重新链接了宿主，最终 EXE 为 `C90991C86DFBE60B9F74577F5C80ABBCE6A15888E44876AE8208723E60D17627`，不把它写成用户此前实际运行的同一二进制。
- `scripts/package_steam.ps1 -SkipBuild` 退出码 0，11.46 秒；Steam 包 398 个文件，清单逐项大小和哈希核对通过，包内主程序与上述全量测试候选相同，版本 `1.0.7.0`、App/Depot `5080330/5080331`、runtime ID `1.0.7.0-cd3ba85799e632d0`。未包含用户 data、开发用 App ID 文件或官方社区组件源码。
- ZIP SHA256：`1BAAB8107D8048FC92CBDE5B0E17713A4CFA83963F16355B12A11F4EF83BDE7D`。Steam 客户端实际更新和新安装形态仍属于线上测试分支验证范围。
