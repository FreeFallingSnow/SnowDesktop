...
| 多页 | 按用途分开桌面 | `DesktopPages` / `pages.order`；说明单屏与多屏的分页方式 |
| 多页 | 新增与管理页面 | 桌面指引；相关设置 → `DesktopPages` / `pages.order` |
| 多页 | 在屏幕边缘换页 | 桌面指引；缺少页面时可打开 `pages.add` |
| 多页 | 拖着内容换页 | 桌面指引；缺少页面时可打开 `pages.add` |
| 多页 | 用键盘翻页 | `DesktopPages` / `general.pageNavigation`；不自动启用 |
...
- 五个主题使用一层原生标签，横向滚动条隐藏而滚动保留。每页一张有界列表，无操作/设置分组。详细说明按需展开；窄窗口按钮换行。
...
- 跨页设置入口使用固定的返回条幅，与卡片等宽，右侧可关闭；通用页无需条幅。目标整张卡片闪烁三次后撤去高亮，刷新不重播；切页、关闭条幅或隐藏窗口时清理。主滚动条位于完整视口右侧，阅读内容的宽度限制保留。

## 验证边界

调用链：设置宿主 → 指南动作/既有设置路由；指南声明 → 桌面只读说明 → 宿主指针与绘制。设置进程通过私有 IPC 请求宿主查询和打开工坊；桌面收起通过异步 IPC 请求设置进程检查最小化状态后正常退出。持久化仍仅折叠偏好，私有 IPC 11 携带会话内返回路由，公共组件 API 不变。

受影响回归包括设置窗口规则、设置控制器/IPC、WinUI 通用页与组件边界、本地化、资源打包。上一轮的正文滚动与前置条件验证不替代本轮窗口复用、返回路由与跨页高亮验证；标准构建与最终全量结果记录于 [实现记录](start_page_onboarding.md)。
